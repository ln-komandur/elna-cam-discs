/*  3D Prints a "single cam" similar to Cam No 16 of Elna Supermatic, Elna SU machines sewing machines.
...
module print_s16_cam(x,y) {
	...
	translate([x, y, 0]) rotate(a=[0,0,0]) difference() {
		...
		for (letter_depth = [1:6])
		{
			...
			rotate(a=[0, 0, 115]) translate([-4, -16, disc_thickness-0.1*letter_depth]) scale(0.002) letter("1");
			rotate(a=[0, 0, 115]) translate([1, -16,disc_thickness-0.1*letter_depth]) scale(0.002) letter("6");
		}
