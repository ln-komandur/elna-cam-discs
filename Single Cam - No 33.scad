/*  3D Prints a "single cam" similar to Cam No 33 of Elna Supermatic, Elna SU machines sewing machines.
...
disc_thickness = 7; // 7 for single, 9 for double
thickest_cyl_rad = 17;
...
module print_s33_cam(x,y) {
	...
	translate([x, y, 0]) rotate(a=[0,0,0]) difference() {
		...
		for (letter_depth = [1:6])
		{
			...
			rotate(a=[0, 0, 115]) translate([-9, -15, disc_thickness-0.1*letter_depth]) scale(0.002) letter(" ");
			rotate(a=[0, 0, 115]) translate([-4, -16, disc_thickness-0.1*letter_depth]) scale(0.002) letter("3");
			rotate(a=[0, 0, 115]) translate([2, -16,disc_thickness-0.1*letter_depth]) scale(0.002) letter("3");
		}
