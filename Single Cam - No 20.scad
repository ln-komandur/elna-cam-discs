/*  3D Prints a "single cam" similar to Cam No 20 of Elna Supermatic, Elna SU machines sewing machines.
...
module print_s20_cam(x,y) {
	...
	translate([x, y, 0]) rotate(a=[0,0,0]) difference() {
		...
		for (letter_depth = [1:6])
		{
			...
			rotate(a=[0, 0, 115]) translate([-7, -15, disc_thickness-0.1*letter_depth]) scale(0.002) letter("2");
			rotate(a=[0, 0, 115]) translate([-2, -16, disc_thickness-0.1*letter_depth]) scale(0.002) letter("0");
			rotate(a=[0, 0, 115]) translate([4, -14,disc_thickness-0.1*letter_depth]) scale(0.002) letter(" ");
		}
